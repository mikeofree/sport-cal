...
CACHE = {
    ...
    "mlb": {"last_fetch": None, "ical_bytes": None},
}
...
def build_calendar(events, prodid: str, uid_prefix: str, default_hours: int) -> bytes:
    ...
    for ev in events.values():
        ...
        summary = f"{uid_prefix.upper()}: {away_name} @ {home_name}"
...
def fetch_league_events(league: str):
    ...
    if league == "nfl":
        ...
        sport_path = "baseball"
        default_hours = 3
        prodid = "-//Homepage Sports Calendar//MLB//EN"
        uid_prefix = "mlb"
    else:
...
@app.route("/mlb.ics")
def mlb_ics():
    ical_bytes = get_cached_calendar("mlb")
    return Response(ical_bytes, mimetype="text/calendar")
...
def root():
    return (
        "Sports ICS server is running. Available feeds: /nfl.ics, /nba.ics, /mlb.ics\n",
        200,
